 ...
 * bundle load order.
 *
 * Shared on purpose: the host list is a union over every sales channel, too
 * expensive to rebuild per injection for a value only an operator changes. The
 * validator is `final` with a readonly host list, so it can be made neither
 * `kernel.reset`-aware nor usefully unshared; `UcpConfigService::saveConfig()`
 * asks the messenger workers to restart instead.
 *
 * @internal
 */
