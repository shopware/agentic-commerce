...
final readonly class CheckoutWebhookUrlGuard
{
    ...
    public function assertAllowed(string $webhookUrl, UcpConfig $config, SalesChannelContext $salesChannelContext): void
    {
        ...
        if (!\is_string($scheme) || !\in_array(strtolower($scheme), ['http', 'https'], true) || !\is_string($host) || '' === $host) {
            throw new ValidationException(
                'Webhook override URLs must use http or https and include a host.',
                ['$.webhookUrlOverride must be an absolute http(s) URL'],
            );
        }
        ...
        if ([] === $allowedHosts) {
            throw new ValidationException(
                'Webhook override host validation requires either agentAllowlist, platformAllowlist, or a resolvable sales channel domain.',
                ['$.webhookUrlOverride host cannot be validated because no allowlist is configured'],
            );
        }

        if (!\in_array(strtolower($host), $allowedHosts, true)) {
            throw new ValidationException(
                sprintf('Webhook override host "%s" is not permitted.', $host),
                ['$.webhookUrlOverride host is not allowlisted'],
            );
        }
    }
}
