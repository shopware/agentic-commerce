...
class OpenAiProductExportProviderTest extends TestCase
{
    ...
    {
        $domain = SwagAgenticCommerce::OPEN_AI_PRODUCT_EXPORT_CONFIG_DOMAIN;
        ...
        $provider = new OpenAiProductExportProvider(
            ...
            $this->createSystemConfigService([
                $domain.'.returnPolicyUrl' => ' https://returns.example/policy ',
    ...
    private function createSystemConfigService(array $config, ?string $expectedSalesChannelId): SystemConfigService
    {
        ...
        $systemConfigService->expects(static::once())
            ->method('getDomain')
            ->with(SwagAgenticCommerce::OPEN_AI_PRODUCT_EXPORT_CONFIG_DOMAIN, $expectedSalesChannelId, true)
            ->willReturn($config);
