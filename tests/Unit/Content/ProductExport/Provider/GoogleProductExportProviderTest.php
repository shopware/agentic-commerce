...
class GoogleProductExportProviderTest extends TestCase
{
    ...
    {
        $domain = SwagAgenticCommerce::GOOGLE_PRODUCT_EXPORT_CONFIG_DOMAIN;
        ...
        $provider = new GoogleProductExportProvider(
            ...
            $this->createSystemConfigService([
                $domain.'.variantColor' => [' color ', '', 5, 'secondary-color'],
                $domain.'.variantSize' => [],
                $domain.'.variantSizeSystem' => ['eu_size'],
                $domain.'.variantGender' => ['unisex'],
                $domain.'.variantAgeGroup' => ['adult'],
                $domain.'.variantMaterial' => ['cotton'],
                $domain.'.variantCondition' => ['refurbished'],
                $domain.'.variantCustom' => ['custom_a', null, 'custom_b'],
            ], $salesChannelId)
        );
    ...
    private function createSystemConfigService(array $config, ?string $expectedSalesChannelId): SystemConfigService
    {
        ...
        $systemConfigService->expects(static::once())
            ->method('getDomain')
            ->with(SwagAgenticCommerce::GOOGLE_PRODUCT_EXPORT_CONFIG_DOMAIN, $expectedSalesChannelId, true)
            ->willReturn($config);
