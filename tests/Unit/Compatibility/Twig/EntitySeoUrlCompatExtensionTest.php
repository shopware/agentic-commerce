 ...
 * @internal
 *
 * @deprecated Covers the {@see EntitySeoUrlCompatExtension} backport (Shopware < 6.7.14); remove together
 * with it once the plugin's minimum supported Shopware version is >= 6.7.14.
 */
...
final class EntitySeoUrlCompatExtensionTest extends TestCase
{
    ...
    public function testEntitySeoUrlResolvesRouteFromRegistryAndDelegatesToSeoUrl(
        ...
        $extension = new EntitySeoUrlCompatExtension($registry, $seoUrlFunctionExtension);

        static::assertSame('GENERATED_PLACEHOLDER', $extension->entitySeoUrl($entityName, $primaryKey, $parameterName));
    }

    public function testEntitySeoUrlReturnsEmptyStringWhenNoRouteIsRegistered(): void
    {
        $registry = $this->createMock(SeoUrlRouteRegistry::class);
