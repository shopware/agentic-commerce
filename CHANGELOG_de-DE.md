...
- Das UCP SDK von Shopware installieren lassen, statt es im Archiv mitzuliefern. 1.3.0 legte das SDK in das plugin-eigene `vendor/` und lud den von Composer dafür erzeugten Autoloader, weil Shopware das `vendor/autoload.php` eines Plugins nicht lädt. Jeder Shop, der das Archiv installiert hatte, betrieb damit zwei Composer-Registries: FroshTools meldete `2 autoloaders registered`, und Composers zusammengeführte Versionsabfragen konnten aus der Kopie des Plugins statt aus der des Shops antworten. Die Erweiterung liefert jetzt überhaupt keine Abhängigkeiten mehr mit und lässt Shopware ihre Anforderungen bei Installation und Update auflösen -- genau dafür ist `composer require` bei einem per ZIP installierten Plugin da: Das Plugin wird über das Pfad-Repository `custom/plugins/*` aufgelöst, das jedes Shopware-Projekt deklariert, und das exakt gepinnte SDK kommt von Packagist in das `vendor/` des Shops. Für einen Shop, der die Erweiterung über Composer installiert, ändert sich nichts. Die Installation braucht nun Zugriff auf Packagist: Ein Shop ohne Internetzugang zur Installationszeit erhält einen Composer-Fehler statt einer funktionierenden Erweiterung.

- Die Erweiterung schaltet sich ab, statt den Shop lahmzulegen, wenn das SDK fehlt. Ein Update entpackt die neuen Dateien einen Request bevor Shopware Composer ausführt, eine aktive Erweiterung startet also mindestens einmal ohne die Abhängigkeit, die sie braucht -- und in 1.3.0 antwortete daraufhin jede Seite des Shops, inklusive Storefront, mit `500`, bis jemand das SDK von Hand installierte. Die Erweiterung registriert in diesem Zustand keine Services, Routen oder Feeds mehr und schreibt nach `var/log/swag-agentic-commerce.log`, was passiert ist und mit welchem Befehl es sich beheben lässt. Sobald das SDK da ist, kommt sie von selbst zurück. Dieselbe Prüfung vergleicht das installierte SDK mit der Version, die dieses Release pinnt: Ein Shop, der noch die vorherige hat, wartet, statt neuen Code gegen ein altes SDK laufen zu lassen.
