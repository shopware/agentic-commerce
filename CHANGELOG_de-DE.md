...
- Optional lässt sich eine verbindlich gewählte Zahlungsart verlangen, bevor eine Bestellung angelegt wird. Ist `requireCommittedPaymentMethod` für einen Verkaufskanal aktiv und hat der Agent keine Zahlungsart benannt, die der Shop abwickeln kann, wird der Checkout mit `requires_escalation` und einer `continue_url` beantwortet, damit ein Mensch im Browser abschließen kann; eine Bestellung entsteht nicht. Standardmäßig aus: Zahlung bleibt wie in der Spezifikation optional, und ohne ausdrückliche Aktivierung ändert sich nichts.
