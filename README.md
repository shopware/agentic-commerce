...
A store release is the one case that differs. A zip-installed plugin has no way to resolve those packages, so `bin/ci-bundle-sdk-only.sh` vendors them into the archive's own `vendor/` and declares their namespaces in the archive's `autoload.psr-4`, which Shopware registers on the shop's class loader (`KernelPluginLoader::registerPluginNamespaces`). The archive ships no autoloader of its own: one would register a second Composer `ClassLoader` in the shop, which is reported as a duplicate autoloader and can shadow the shop's own package versions. `vendor/composer/installed.json` is the only Composer file that ships, because Shopware's `RequirementsValidator` reads it to satisfy the `ucp-php-sdk/*` requirements.
