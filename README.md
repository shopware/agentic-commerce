...
| Product feed | Outbound product feed surface for agentic/catalog consumers. | Feed consumers, marketplaces, AI catalog ingestion, merchants managing feed availability. | Implemented through Shopware product exports with Agentic Commerce sales-channel templates, providers, validation, and tracking. |
...
- UCP routes are provided by `ucp-php-sdk/symfony-bundle`; the plugin supplies Shopware adapters, sales-channel configuration, and lane-aware capability exposure.
- The SDK is configured with strict signature policy, required idempotency, ES256 signing keys, key retirement, replay/idempotency storage, and profile fetching through the plugin runtime.
- REST, A2A, embedded, and MCP transports share the same capability adapters. MCP is advertised only when the current Shopware lane exposes the required Store API MCP primitives.
- Embedded pages require configured allowed origins and frame ancestors. Missing or disallowed origins are rejected before rendering.
- Unsupported capabilities are omitted from published profiles instead of being advertised as inactive examples.
...
- `/agents.md` and `/llms.txt` are rendered through core sales-channel files when available.
- On lanes without core sales-channel files, the plugin registers fallback storefront routes for the same agent-facing documents.
- The plugin contributes UCP-specific sections through `src/Resources/views/agentic-commerce/includes/ucp-agents.md.twig` and `src/Resources/views/agentic-commerce/includes/ucp-llms.txt.twig`.
- Discovery output stays sales-channel aware and should only mention transports and capabilities that are available for the current lane and configuration.
...
Product feeds reuse Shopware's product export/product comparison infrastructure instead of adding a separate feed runtime. The plugin adds Agentic Commerce sales-channel templates, a `provider` field for lanes that do not ship it natively, provider-specific render context, JSONL normalization, provider validators, and sales-channel referral tracking.

Supported providers and formats:

- `open-ai`: JSON Lines (`jsonl`) feed rendered with the OpenAI template and served as `application/x-ndjson`.
- `google`: XML feed rendered with the Google product export template.

Administration and configuration:

- Product export templates are registered in the Administration for the Agentic Commerce sales-channel type.
- Provider-specific settings live in Shopware system config under `SwagAgenticCommerce.openAiProductExport` and `SwagAgenticCommerce.googleProductExport`.
- Variant mappings use property groups and always pick the first matching configured property, with custom mappings limited by the provider template.
- The OpenAI provider requires a return policy URL and validates fields such as `item_id`, title, description, brand, URLs, price, country codes, availability, and target countries.
- The Google provider validates the XML feed against the provider-specific rules and emits Google-compatible variant fields.

Generation, cache, and invalidation:

- Feed URLs, access, generation, scheduling, and cache invalidation follow Shopware's product export behavior for the configured product export.
- The shipped Agentic Commerce templates default to manual generation (`generateByCronjob: false`) with a 24-hour interval value (`86400`) when cron generation is enabled by the merchant.
- JSONL output is normalized to one JSON object per non-empty line and URL spaces are encoded before validation.
- Storefront URLs include the `referringSalesChannel` query parameter so visits, customers, and orders can be attributed back to the Agentic Commerce sales channel.
- Feed generation is independent of UCP transports. UCP profiles and discovery documents may link to feed locations, but feed rendering does not depend on UCP request handling.

QA:

- Create or reset an Agentic Commerce product export from the Administration template.
- Generate the feed and check that the selected provider validator reports no errors.
- Open the generated URL for each relevant sales-channel domain and verify product visibility, absolute URLs, locale/currency output, and provider-specific fields.
- Follow a product URL containing `referringSalesChannel` and verify customer/order tracking when the storefront session converts.
