...
- Optionally require a committed payment method before an order is placed. With `requireCommittedPaymentMethod` turned on for a sales channel, a completion where the agent has committed no handler the shop can settle is answered with `requires_escalation` and a `continue_url` so a person can finish in the browser, and no order is placed. Off by default: payment stays optional as the specification has it, and nothing changes until a merchant opts in. It exists for agent-only channels that must not accumulate orders nobody can pay.
