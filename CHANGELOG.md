...
- Let Shopware install the UCP SDK instead of shipping it inside the archive. 1.3.0 vendored the SDK into the plugin's own `vendor/` and loaded the autoloader Composer generated for it, because Shopware does not load a plugin's `vendor/autoload.php`. Every shop that installed it then ran two Composer registries: FroshTools reported `2 autoloaders registered`, and Composer's aggregated version lookups could answer from the plugin's copy rather than the shop's. The extension now carries no dependencies at all and asks Shopware to resolve its requirements when it is installed or updated, which is what `composer require` on a zip-installed plugin is for -- the plugin resolves from the `custom/plugins/*` path repository every Shopware project declares, and the pinned SDK comes from Packagist into the shop's own `vendor/`. A shop that installs the extension through Composer is unaffected. The install now needs to reach Packagist: a shop with no outbound network at install time gets a Composer error instead of a working extension.

- Switch the extension off instead of taking the shop down when the SDK is missing. An update extracts the new files one request before Shopware runs Composer, so an active extension boots at least once without the dependency it needs -- and on 1.3.0 that meant every page of the shop, storefront included, answered `500` until someone installed the SDK by hand. The extension now registers no services, routes or feeds in that state and writes what happened, and the one command that fixes it, to `var/log/swag-agentic-commerce.log`. It comes back on its own as soon as the SDK is there. The same check compares the installed SDK against the version this release pins, so a shop still holding the previous one waits rather than running new code against an old SDK.
